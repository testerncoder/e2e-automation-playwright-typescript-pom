...
test.describe('User SignIn Tests: ', () => {
    ...
    test('login with valid creds', async () => {
        try {
            ...
            const pageTitle = await loginPage.eleSignInFormTitle()
            expect(await pageTitle?.textContent()).toContain('Sign in')
            ...
            await page.screenshot({
                path: `screenshots/${await myAccount.hiUserWindow()}.png`,
                fullPage: false,
            })
            await page.close()
        } catch {
